...
\ty{driver.sh}. It processes the same list of type strains as
\ty{driver.sh},
\begin{verbatim}
$ bash anam.sh < list.txt
\end{verbatim}
...
\ty{keyMat} has then has the repeating format
\begin{verbatim}
# header
[position accession]
# header
...
#+end_src
#+begin_export latex
To extract the amplicon coordinates, we the marker harboring the
...
#+end_src
#+begin_export latex
The amplicon end on the marker, $e$, is a function of \ty{posR}, $p$,
and the marker length, $\ell$:
\[
e = \ell - p + 1.
\]
To find the marker length, we get its sequence from the file
\ty{markers2.fasta} using \ty{getSeq} and measure the sequence length
with \ty{cres}.
#+end_export
#+begin_src sh <<Find amplicon end, Pr. \ref{pr:anam}>>=
  l=$(getSeq $name markers2.fasta |
      cres |
      grep '^To' |
      awk '{print $2}')
  ((markerEnd=$l-$posR+1))
...
#+end_src
#+begin_export latex
To determine an assembly accession, we list the target files and store
the name of the first one, for example,
\begin{verbatim}
...
#+end_src
#+begin_export latex
We place the amplicon on the assembly using \ty{sblast} and determine
its start and end there.
#+end_export
#+begin_src sh <<Place amplicon on assembly, Pr. \ref{pr:anam}>>=
  br=$(sblast amplicon.fasta assembly.fasta |
      tail -n +2 |
      head -n 1)
  start=$(echo $br | awk '{print $5}')
  end=$(echo $br | awk '{print $6}')
#+end_src
#+begin_export latex
...
annotations, we note that the GFF file contains start and end
positions of annotations in columns 4 and 5. We select the annotations
that intersect our amplicon and save them also to \ty{anam.txt}. The
first annotation is always the full ``region'' containing all other
annotations. Although this is redundant, we include it as a positive
control.
#+end_export
#+begin_src sh <<Look up annotations, Pr. \ref{pr:anam}>>=
  echo "#Annotation for interval ($start, $end)" > anam.txt
  awk -v s=$start -v e=$end '$4<=e && $5>=s' \
      genomic.gff |
      grep -v '^#' >> anam.txt
#+end_src
